...
- Geen wijziging aan applicatiecode, Docker-/Compose-configuratie, deploymentconfiguratie of runtime-omgeving als onderdeel van deze spec-update; eventuele hardening blijft een reeds beoordeeld implementatieonderdeel dat alleen als release-eis en evidence wordt vastgelegd.
...
Implementatie is goedgekeurd na review; de in-scope reparatie voor backend-werkdirectorycommandoguidance is voltooid. Runtime-diagnose, artefactopbouw/-inspectie met vastgelegde identity en dirty-state, release, productiemigratie en productievalidatie blijven pending operator-evidence en afzonderlijke autorisatie.
...
- Artefactregistratie-eis toegevoegd: `13774235` is uitsluitend de reeds gemelde base identity, geen vastgelegde finale releasecommit of image digest. De finale immutable commit/ref, eventuele image digest, buildtijd en clean/dirty-state zijn nog niet als evidence aangeleverd en blijven pending.
...
- Geïmplementeerd: `scripts/build_isolated_release_artifact.py` eist een volledige immutable commit-SHA, exporteert die met `git archive` naar een tijdelijke buildcontext en bouwt daar de drie runtime-images. Daardoor kan geen dirty worktree of lokaal bestand deel uitmaken van een releaseartefact. `scripts/verify_docker_image_isolation.py` exporteert een image zonder hem te starten en blokkeert uitgesloten applicatiepaden; de regressietest dekt iedere uitgesloten categorie.
...
Lokaal, vanuit de geïsoleerde releasebron (base `13774235`); iedere backend-opdracht start expliciet vanuit `backend/`:
...
Een release build gebruikt daarna uitsluitend `cd backend && .venv/bin/python scripts/build_isolated_release_artifact.py <volledige-commit-SHA>` en registreert de uitvoer. De lokaal gebouwde `:isolation-r2` images zijn alleen verificatieartefacten uit deze dirty worktree en niet vrijgavegeschikt.
...
---
Status: implementation completed locally; immutable releaseartefactevidence, runtime operator-evidence en productie-release blijven pending
Owner: —
Date: 2026-08-12
